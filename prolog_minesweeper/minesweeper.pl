...
Предикат minus_one/2 используется для ограничения количества соседей для любой клетки на
поле сапера до 8.
Каждый вызов minus_one/2 уменьшает первый аргумент на 1 и присваивает
результат второму аргументу. */
minus_one(8, 7).
minus_one(7, 6).
minus_one(6, 5).
minus_one(5, 4).
...
Предикат first_column/3 принимает список списков и возвращает первый элемент каждого
подсписка в виде списка,
...
Клетка может быть уменьшена, если она является миной или если это число,
которое равно количеству мин в соседних клетках.*/
tile_reduces(*, _).
tile_reduces(0, []).
